...
def delay_lost(dm: float, chan_freqs: List[float], tsamp: float) -> np.int64:
    """
    ...
    return -np.round(max_delay / tsamp).astype(np.int64)


def calc_dispersion_delays(dm: float, chan_freqs: List[float]) -> List[float]:
    """
