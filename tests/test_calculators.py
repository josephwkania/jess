...
class TestAccumulate:
    ...
    def test_pad(self):
        """
        Test accumulate on axis=1 with padding using the edge value.
        """
        padded_to_accumulate = np.pad(self.to_accumulate, ((0, 1), (0, 0)), mode="edge")
        accumulates_hor = calc.accumulate(
            padded_to_accumulate, factor=2, axis=1, pad="edge"
        )
        assert np.array_equal(
            accumulates_hor,
            np.asarray(
